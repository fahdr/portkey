...
resource "cloudflare_tunnel" "portkey" {
...
resource "cloudflare_record" "tunnel" {
  ...
  type    = "CNAME"
  name    = "portkey-tunnel"
  value   = "${cloudflare_tunnel.portkey.id}.cfargotunnel.com"
  proxied = false
  ttl     = 1 # Auto
}
...
resource "kubernetes_secret" "cloudflared_credentials" {
  ...
  data = {
    "credentials.json" = jsonencode({
      ...
      TunnelName   = cloudflare_tunnel.portkey.name
      TunnelID     = cloudflare_tunnel.portkey.id
